...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_BLOG: 'blog/PARTIAL_UPDATE_BLOG',
...
export default (state: BlogState = initialState, action): BlogState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_BLOG):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_BLOG):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_BLOG):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const partialUpdate: ICrudPutAction<IBlog> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_BLOG,
